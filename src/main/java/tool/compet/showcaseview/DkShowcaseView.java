 ...
 *
 * Ref: https://github.com/mreram/ShowCaseView
 */
public class DkShowcaseView extends RelativeLayout implements View.OnTouchListener, ShowcaseViewApi {
	...
	protected DkShowcaseView(Context context, boolean newStyle) {
		this(context, null, R.styleable.CustomTheme_showcaseViewStyle, newStyle);
	}

	protected DkShowcaseView(Context context, AttributeSet attrs, int defStyle, boolean newStyle) {
		super(context, attrs, defStyle);

		MyApiUtils apiUtils = new MyApiUtils();
		if (apiUtils.isCompatWithHoneycomb()) {
			animationFactory = new MyAnimatorAnimationFactory();
		}
		else {
		...
		mEndButton = (Button) LayoutInflater.from(context).inflate(R.layout.dkshowcaseview_showcase_button, null);
		if (newStyle) {
			showcaseDrawer = new MyNewShowcaseDrawer(getResources(), context.getTheme());
		}
		else {
			showcaseDrawer = new StandardShowcaseDrawer(getResources(), context.getTheme());
		}
		textDrawer = new MyTextDrawer(getResources(), getContext());
	...
	public void setTarget(final MyTarget target) {
		setShowcase(target, false);
	}

	public void setShowcase(final MyTarget target, final boolean animate) {
		postDelayed(() -> {
			if (!shotStateStore.hasShot()) {
				...
				if (targetPoint != null) {
					hasNoTarget = false;
					if (animate) {
						animationFactory.animateTargetToPoint(DkShowcaseView.this, targetPoint);
					}
					else {
	...
	private void fadeOutShowcase() {
		animationFactory.fadeOutView(this, fadeOutMillis, () -> {
			setVisibility(View.GONE);
			clearBitmap();
			isShowing = false;
			mEventListener.onShowcaseViewDidHide(DkShowcaseView.this);
		});
	}
	...
	private void fadeInShowcase() {
		animationFactory.fadeInView(this, fadeInMillis, () -> setVisibility(View.VISIBLE));
	}
	...
	private static void insertShowcaseView(DkShowcaseView showcaseView, ViewGroup parent, int parentIndex) {
	...
	/**
	 * Builder class which allows easier creation of {@link DkShowcaseView}s.
	 * It is recommended that you use this Builder class.
	 */
	public static class Builder {

		private final DkShowcaseView showcaseView;
		...
		public Builder(Activity activity, boolean useNewStyle) {
			...
			this.showcaseView = new DkShowcaseView(activity, useNewStyle);
			this.showcaseView.setTarget(MyTarget.NONE);
			this.parent = activity.findViewById(android.R.id.content);
			this.parentIndex = parent.getChildCount();
		}

		/**
		 * Create the {@link DkShowcaseView} and show it.
		 *
		 * @return the created ShowcaseView
		 */
		public DkShowcaseView build() {
		...
		public Builder withNewStyleShowcase() {
			return setShowcaseDrawer(new MyNewShowcaseDrawer(activity.getResources(), activity.getTheme()));
		}
		...
		public Builder withMaterialShowcase() {
			return setShowcaseDrawer(new MyMaterialShowcaseDrawer(activity.getResources()));
		}
		...
		public Builder setTargetView(View view) {
			return setTarget(new MyViewTarget(view));
		}

		/**
		 * Set the target of the showcase.
		 *
		 * @param target a {@link MyTarget} representing
		 *               the item to showcase (e.g., a button, or action item).
		 */
		public Builder setTarget(MyTarget target) {
	...
	/**
	 * @see DkShowcaseView.Builder#setSingleShot(long)
	 */
	...
	/**
	 * @see DkShowcaseView.Builder#hideOnTouchOutside()
	 */
	@Override
	...
	/**
	 * @see DkShowcaseView.Builder#doNotBlockTouches()
	 */
	@Override
	...
	/**
	 * @see DkShowcaseView.Builder#setStyle(int)
	 */
	@Override
